function [indices,duraciones,luces]=hip2csv(h1,lon_epoca,arch,gen,codigo,luz,modoarch)
...
% modoarch 1 con cabecero-inicio 2 sin cabecero-append
  ...
  estadios=[w_estadios;n_estadios;r_estadios];
   ...
   if(modoarch==1)
       ...
       fprintf(fileID,'%s\t%s\t%s\t%s\t%s\t%s\n','cod','GEN','dur','est','t','luz');
   else
       fileID = fopen([arch '.csv'],'a');
   end
       
   for(ii=1:length(indices))
       fprintf(fileID,'%s\t%s\t%d\t%d\t%d\t%s\n',codigo,gen,duraciones(ii)*lon_epoca,estadios(ii),indices(ii)*lon_epoca,luztxt{luces(ii)});
   end    
   fclose(fileID);
